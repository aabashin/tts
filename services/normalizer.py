...
class Normalizer:
    ...
    Uses neural network torch model that normalize numbers with context cases\n
    Long text split on chunks and predict word written numbers in text by the model\n
    -------
    Methods:
    -------
    ...
    def __init__(self, device="cpu") -> None:
        ...
        self.model = torch.jit.load(
            settings.NUM_TO_TEXT_MODEL, map_location=settings.DEVICE
        )
        self.model.eval()
        self.max_len = 150

    def init_vocabs(self) -> None:
        ...
        self.src_vocab = {
            ...
            for i, token in enumerate(
                printable[:-5] + rus_letters + "«»—" + spec_symbols
            )
        }
    ...
    def keep_unknown(self, string: str) -> tuple:
    ...
    def norm_text(self, text: str) -> str:
        ...
        splitters = "\n\t?!"
        parts = [
            p for p in re.split(r"({})".format("|\\".join(splitters)), text) if p != ""
        ]
        norm_parts = []
                    ...
                    while start_point < len(part):
                        if curr_point in spaces:
                            if (
                                ...
                                norm_parts.append(
                                    self._norm_string(part[start_point:end_point])
                                )
